...
export function FaqSection() {
  ...
  return (
    <section id="faq" className="relative overflow-hidden bg-white px-4 py-24 sm:px-6 md:px-10 md:py-32">
      ...
      <div className="relative z-10 mx-auto grid max-w-[72rem] gap-10 lg:grid-cols-[0.72fr_1.28fr] lg:gap-16">
        <div>
          <p className="text-label text-[#c8102e]">08 / {t('faqKicker')}</p>
        ...
        <Accordion className="rounded-[2rem] bg-[#f4faff] p-2 shadow-[0_18px_45px_rgba(20,52,100,0.12)] sm:p-4">
          {FAQS.map(({ q, a }, index) => (
            <AccordionItem key={q} value={q} className="border-navy/10 px-3 sm:px-4">
              ...
              <AccordionContent className="pb-5 pl-6 text-sm font-medium leading-6 text-navy/75 sm:pl-7">{t(a)}</AccordionContent>
            </AccordionItem>
          ))}
        </Accordion>
      </div>

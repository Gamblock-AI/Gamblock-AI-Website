...
| Admin control plane | implemented | Content CMS, tickets, release catalog, emergency, platform |
| Public download catalog | implemented | Landing download cards load published bilingual Android, Windows, and passive-browser-extension release metadata from `GET /v1/public/download-apps`; verified admins update allowlisted official GitHub Release asset URLs, sizes, SHA-256 checksums, and publication state through the audited Releases workspace. The catalog never receives browsing or user telemetry. |

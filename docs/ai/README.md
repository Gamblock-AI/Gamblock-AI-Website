...
|---|---|---|
| Dashboard (student/partner/admin) | implemented | Per-role surface terpisah di `/dashboard`. Student: sapaan, strip ringkasan (check-in minggu, blokir proteksi, hari aktif, streak), langkah edukasi selanjutnya, bantuan darurat, **chart tren blokir proteksi 7 hari** (dari `/client/progress?days=7`), ringkasan proteksi, FAB misi harian, dan **kartu rekomendasi SPK harian** (`SpkRecommendationCard`): **nama fitur** yang paling cocok hari ini dari `GET /v1/client/spk-recommendation` (headline LLM jadi subtitle kutipan), CTA ke route terkait, blok **"Alasan rekomendasi"** (chip `Kebutuhan dukungan`/`Aktivitas pemulihan` + penjelasan LLM bila aktif atau kalimat `reason.code` dengan **nama fitur terinterpolasi** + faktor + pola jam rawan), dan **peringatan data terbatas** dengan daftar `data_gaps` yang bisa diisi (belajar→`/skills`, check-in/misi→`/recovery`, niat) + tombol "Muat ulang" (refetch) atau catatan `/settings` saat privasi dimatikan; kartu disembunyikan saat endpoint error dan menampilkan skeleton saat loading. Tombol "tandai selesai" dihapus dari kartu; endpoint `POST /v1/client/spk-interventions/:id/complete` tetap tersedia di backend sebagai kontrak untuk auto-complete di masa depan. State `recommendation_enabled=false` menampilkan catatan "rekomendasi dimatikan" + tautan `/settings` |

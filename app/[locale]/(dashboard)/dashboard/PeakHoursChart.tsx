  ...
  return (
    <Card className="p-5 transition-shadow hover:shadow-card">
      ...
      <div className="mt-4 space-y-3">
        <div className="group flex h-28 items-end justify-between gap-1.5 md:gap-3.5">
          {peakData.map((val, idx) => {
            ...
            return (
              <div key={idx} className="flex h-full flex-1 flex-col items-center justify-end gap-3">
                <div
                  className={`w-full rounded-full transition-all duration-300 hover:opacity-80 ${
                    isPeak ? 'bg-crimson' : 'bg-navy/15 group-hover:bg-navy/25'
                  }`}
                  style={{ height: `${val}%` }}
                />
              </div>

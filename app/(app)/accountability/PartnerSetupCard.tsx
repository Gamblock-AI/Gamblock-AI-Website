    ...
    <div className="space-y-3 rounded-lg border border-slate-100 bg-white p-4 shadow-[0_4px_20px_rgba(0,0,0,0.01)]">
      ...
      {partnerStatus === 'none' ? (
        <div className="space-y-3">
          <div className="space-y-1.5">
            <label className="text-xs font-bold tracking-wider text-slate-500 uppercase">
              Email Calon Pendamping
            </label>
            <p className="text-[11px] text-slate-400">
              Masukkan email pendamping tepercaya Anda (misal: keluarga, teman
              dekat, atau konselor).
            </p>
            <input
              type="email"
              ...
              placeholder="nama.pendamping@example.com"
              disabled={loading}
          ...
          {!isSettingsPage && (
            ...
            >
              <HeartHandshake className="size-4" /> Undang Pendamping
            </button>
          )}
        </div>
      ) : (
        <div className="space-y-3">
          ...
          >
            <Trash2 className="size-3.5" /> Putuskan Hubungan Pendamping
          </button>
        </div>
      )}
    </div>

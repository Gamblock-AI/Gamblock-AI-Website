      ...
      <div className="rounded-lg border border-slate-100 bg-white p-4 shadow-[0_4px_20px_rgba(0,0,0,0.01)]">
        <div className="space-y-1">
          ...
          <p className="text-sm text-slate-500">
            Kelola ekspor riwayat aktivitas lokal Anda atau ajukan penghapusan
            akun sesuai ketentuan UU PDP.
          </p>
        </div>
      </div>
      ...
      <div className="grid gap-3 md:grid-cols-2">
        ...
        <div className="flex flex-col justify-between space-y-3 rounded-lg border border-slate-100 bg-white p-4 shadow-[0_4px_20px_rgba(0,0,0,0.01)]">
          <div className="space-y-3">
            ...
            <p className="text-xs leading-relaxed text-slate-500">
              Unduh semua arsip aktivitas proteksi, log deteksi lokal, riwayat
              sinkronisasi ruleset, dan entri refleksi harian Anda dalam format
              standar JSON/ZIP.
            </p>
          </div>
          ...
          >
            Mulai Ekspor Data <Download className="size-3.5" />
          </button>
        </div>
        ...
        <div className="flex flex-col justify-between space-y-3 rounded-lg border border-slate-100 bg-white p-4 shadow-[0_4px_20px_rgba(0,0,0,0.01)]">
          <div className="space-y-3">
            ...
            <p className="text-xs leading-relaxed text-slate-500">
              Menghapus seluruh identitas akun Anda di server kami secara
              permanen. Tindakan ini memerlukan otorisasi konfirmasi melalui
              email dan tidak dapat dibatalkan.
            </p>
          </div>
          ...
          >
            Hapus Akun Permanen <Trash2 className="size-3.5" />
          </button>
        </div>
      </div>
          ...
          <table className="w-full border-collapse text-left text-sm">
            <thead>
              <tr className="border-b border-slate-100 text-xs font-bold tracking-wider text-slate-400 uppercase">
                ...
                <th className="px-3 py-2.5">Tanggal Pengajuan</th>
                <th className="px-3 py-2.5">Jenis Permintaan</th>
                <th className="px-3 py-2.5">Status</th>
                <th className="px-3 py-2.5">Hasil / Unduh</th>
              </tr>
            </thead>
            <tbody className="font-semibold text-slate-700">
              {requests.length === 0 ? (
                <tr className="border-b border-slate-50">
                  ...
                  >
                    Tidak ada riwayat pengajuan permintaan data
                  </td>
                </tr>
              ) : (
                requests.map((r, idx) => (
                    ...
                    <td className="px-3 py-2.5 text-xs">
                      {r.result === 'Siap Unduh' ? (
                        <span className="text-crimson flex cursor-pointer items-center gap-1 font-bold hover:underline">
                          <Download className="size-3.5" /> Unduh ZIP (1.2 MB)
                        </span>
                      ) : (
